/**
 * Italy Card Game — Client v5
 * Real cards · Particles · Deal animation · Trump explosion · Confetti
 */
const socket = io();
...
function tone(f,d=.1,t='sine',v=.13){
...
function launchConfetti(duration=3200){
  ...
  const ctx=cv.getContext('2d');
  const cols=['#c8941a','#e5b84a','#f5d98e','#5b9cf6','#f47a7a','#80cbc4','#ce93d8','#fff'];
  const pieces=[];
  for(let i=0;i<150;i++){
    pieces.push({
  ...
  (function frame(ts){
    if(!start)start=ts;
    const el=ts-start,fade=Math.max(0,1-(el-duration*.55)/(duration*.45));
    ctx.clearRect(0,0,cv.width,cv.height);
    pieces.forEach(p=>{
...
function showTrumpExplosion(suit){
  const fx=$('trump-fx');
  const sf=$('tf-suit');const st=$('tf-text');
  if(!fx||!sf)return;
  sf.textContent=SYM[suit]||'★';
  st.textContent='TRUMP REVEALED!';
  // Reset animation
...
// ── Bid Timer ─────────────────────────────────────────
const TSECS=30,CIRC=Math.PI*2*21; // r=21
function startBidTimer(onExp){
  clearInterval(bidTimerInterval);
  let left=TSECS;
  const update=()=>{
    const numEl=$('timer-num'),ringEl=$('timer-ring');
    if(numEl){numEl.textContent=left;numEl.className='timer-num'+(left<=8?' warn':'');}
    if(ringEl){ringEl.style.strokeDashoffset=CIRC*(1-left/TSECS);ringEl.className='timer-fg'+(left<=8?' warn':'');}
    if(left<=5&&left>0)sfxTick();
    if(left<=0){clearInterval(bidTimerInterval);if(onExp)onExp();}
    left--;
  };
  update();
  bidTimerInterval=setInterval(update,1000);
}
function stopBidTimer(){
  clearInterval(bidTimerInterval);
  const n=$('timer-num'),r=$('timer-ring');
  if(n){n.textContent=TSECS;n.className='timer-num';}
  if(r){r.style.strokeDashoffset=0;r.className='timer-fg';}
}
  ...
  d.innerHTML=`
    <div class="card-face">
      <div class="card-tl">
        ...
        <div class="card-suit-sm">${SYM[card.suit]}</div>
      </div>
      <div class="card-center"><span class="card-suit-big">${SYM[card.suit]}</span></div>
      <div class="card-br">
        <div class="card-rank">${card.rank}</div>
        <div class="card-suit-sm">${SYM[card.suit]}</div>
      </div>
...
function renderHand(animate=false){
  ...
  myHand.forEach((card,i)=>{
    ...
    if(animate){el.classList.add('dealing');el.style.animationDelay=`${i*58}ms`;}
...
function setTrick(sp,card){
  ...
  el.innerHTML='';
  if(card){const c=mkCard(card,'tc played');el.appendChild(c);}
}
...
function revealTrumpPanel(card){
  ...
  c.style.animation='cardSlide .4s cubic-bezier(.34,1.56,.64,1)';
  sl.appendChild(c);
...
function renderPlayers(ps){
  ps.forEach(p=>{
    ...
    if(avEl){
      ...
      if(inner){
        const em=playerEmojis[p.position];
        inner.textContent=em||p.name[0].toUpperCase();
      }
    }
...
function showDealAnim(dealerName,firstActiveName){
  ...
  for(let i=0;i<8;i++){
    const d=document.createElement('div');d.className='do-c';
    d.style.animationDelay=`${i*72}ms`;row.appendChild(d);
  }
  ov.classList.add('show');setTimeout(()=>ov.classList.remove('show'),2500);
}

// ── Emoji picker ──────────────────────────────────────
function buildEmojiPicker(containerId){
  const c=$(containerId);if(!c)return;
  c.innerHTML='';
  EMOJIS.forEach(em=>{
    const btn=document.createElement('button');
    btn.type='button';btn.className='ep'+(em===myEmoji?' sel':'');
    btn.textContent=em;
    btn.onclick=()=>{
      myEmoji=em;
  ...
  for(let i=0;i<4;i++){
    ...
    if(p){
      ...
      div.innerHTML=`<div class="s-av ${tm}">${em}</div>
        <div class="s-info"><div class="s-num">Seat ${i+1}${isMe?' ★':''}</div>
        <div class="s-name">${p.name}</div></div>
        <span class="s-badge ${tm}">Team ${tm}</span>`;
    }else{
      div.innerHTML=`<div class="s-av empty">＋</div>
        <div class="s-info"><div class="s-num">Seat ${i+1}</div>
        <div class="s-name" style="opacity:.25">Empty</div></div>`;
    }
...
function selectTarget(v){
  ...
  $('t30').classList.toggle('sel',v===30);$('t50').classList.toggle('sel',v===50);
}
...
socket.on('roomCreated',({code,position,players:ps,isHost,emojis})=>{
  ...
  $('disp-code').textContent=code;renderSeats(ps);showScreen('screen-waiting');
...
socket.on('playerJoined',({players:ps,emojis})=>{
  players=ps;applyEmojis(emojis);renderSeats(ps);sfxDeal();
  toast(`${ps[ps.length-1].name} joined!`);
});
socket.on('allReady',({players:ps,emojis})=>{
  players=ps;applyEmojis(emojis);renderSeats(ps);
  toast(amHost?'All 4 ready! You can start.':'All 4 players ready!');
});
socket.on('targetSet',({target})=>{
  matchTarget=target;$('t30')?.classList.toggle('sel',target===30);$('t50')?.classList.toggle('sel',target===50);
...
socket.on('handUpdate',({hand,dealPhase})=>{
  myHand=hand;handCounts[myPos]=hand.length;renderHand(dealPhase==='initial');
});
...
socket.on('powerCardPlaced',({bidderPos,bidderName,bid})=>{
  currentBidder=bidderPos;$('status').textContent=`${bidderName} placed power card (bid:${bid})`;
  toast(`${bidderName} placed power card`);
});
socket.on('callingDone',({bidder,bidderName,bid})=>{
  currentBidder=bidder;$('status').textContent=`${bidderName} wins bid at ${bid}`;
...
socket.on('fullHandDealt',({hand,bidder,bid,powerCardSuit})=>{
  ...
  if(myPos===bidder&&powerCardSuit)
    toast(`Power card: ${SYM[powerCardSuit]} ${powerCardSuit} — keep secret!`,3500);
});
...
socket.on('trickComplete',({winnerPos,winnerName,winnerTeam,tricksWon,trickNumber})=>{
  ...
  setTimeout(()=>sfxWin(),120);
  setTimeout(()=>updateTricks(tricksWon),150);
